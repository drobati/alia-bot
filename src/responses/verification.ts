...
async function getLogChannelId(tables: any, guildId: string): Promise<string | null> {
    const config = await tables.Config.findOne({
        where: { key: `verify_log_channel_${guildId}` }
    });
    return config?.value || null;
}
...
export default async (message: Message, context: Context): Promise<boolean> => {
    ...
    try {
        ...
        if (!role) {
            log.error({ roleId: verificationCode.roleId, guildId: message.guildId }, 'Role not found for verification code');
            return false;
        }
        ...
        const logChannelId = await getLogChannelId(tables, message.guildId);
        if (logChannelId) {
            const logChannel = message.guild.channels.cache.get(logChannelId) as TextChannel;
            if (logChannel && logChannel.isTextBased()) {
                const timestamp = new Date().toISOString().replace('T', ' ').substring(0, 19);
                const logMessage = `[${timestamp}] <@${message.author.id}> used code ${code} (generated by <@${verificationCode.generatorId}>) → granted @${role.name}`;
                try {
