...
export class HoroscopeGenerator {
    private static readonly CONTENT_TEMPLATES = {
        daily: [
            {
                category: 'positive',
                template: "The cosmic energies align beautifully for {sign} today! Your {element} nature will be your greatest strength as {planet} bestows its favor upon you. Expect opportunities in areas requiring your natural {trait1} and {trait2} abilities. The universe conspires to bring you closer to your goals.",
                weight: 3,
            },
            {
                category: 'motivational',
                template: "Today challenges {sign} to embrace their inner {trait1} spirit. As a {element} sign ruled by {planet}, you possess the unique ability to {action}. Trust your instincts, dear {sign}, for they will guide you toward unexpected victories.",
                weight: 3,
            },
            {
                category: 'neutral',
                template: "A day of balance awaits {sign}. Your {element} energy flows steadily, encouraging practical decisions and meaningful connections. {planet}'s influence reminds you to stay grounded while remaining open to new possibilities.",
                weight: 2,
            },
            {
                category: 'reflective',
                template: "The stars invite {sign} to pause and reflect today. Your {trait1} nature has carried you far, and now it's time to appreciate the journey. {planet} whispers wisdom about the importance of both action and rest.",
                weight: 1,
            },
        ],
        love: [
            {
                category: 'romantic',
                template: "Venus dances through your heart sector, dear {sign}! Your magnetic {trait1} energy draws romantic possibilities like a cosmic magnet. If partnered, expect deeper intimacy. If single, your {trait2} charm will captivate someone special. Love flows through your {element} essence today.",
                weight: 4,
            },
            {
                category: 'relationship',
                template: "Relationships take center stage for {sign} today. Your natural {trait1} approach to love creates harmony and understanding. {planet}'s energy encourages honest communication and emotional vulnerability. Let your heart lead with {trait2} confidence.",
                weight: 3,
            },
            {
                category: 'self-love',
                template: "The greatest love story for {sign} today begins with self-appreciation. Your {element} spirit deserves recognition for its strength and beauty. Practice self-care rituals that honor your {trait1} nature and nurture your inner flame.",
                weight: 2,
            },
        ],
        career: [
            {
                ...
                template: "Professional opportunities shimmer on the horizon for {sign}! Your reputation for being {trait1} and {trait2} opens doors previously unseen. {planet}'s energy amplifies your natural leadership abilities. Bold decisions made today will pay dividends in the future.",
                weight: 4,
            },
            {
                category: 'collaboration',
                template: "Teamwork and collaboration are your superpowers today, {sign}. Your {element} energy harmonizes beautifully with colleagues, creating innovative solutions. Use your {trait1} skills to bridge gaps and build lasting professional relationships.",
                weight: 3,
            },
            {
                category: 'skills',
                template: "Time to showcase your unique talents, {sign}! Your {trait1} abilities combined with your natural {trait2} approach sets you apart from the crowd. {planet} encourages you to step into the spotlight and claim your professional worth.",
                weight: 3,
            },
        ],
        lucky: [
            {
                category: 'numbers',
                template: "The cosmic lottery smiles upon {sign} today! Lucky numbers flow from {planet}'s numerical vibrations, while fortune favors your {trait1} endeavors. Wear or surround yourself with your power colors to amplify this magnificent energy.",
                weight: 3,
            },
            {
                category: 'timing',
                template: "Perfect timing is {sign}'s secret weapon today! Your {element} intuition will guide you to seize the right moments for maximum impact. Watch for signs and synchronicities - the universe is speaking directly to you.",
                weight: 3,
            },
        ],
        weekly: [
            {
                category: 'overview',
                template: "This week unfolds like a beautiful tapestry for {sign}, woven with threads of opportunity and growth. Your {trait1} nature will be tested and strengthened, while {planet}'s weekly transit brings unexpected gifts. Embrace change with your characteristic {trait2} spirit.",
                weight: 3,
            },
            {
                category: 'challenges',
                template: "The week ahead presents {sign} with meaningful challenges that will ultimately strengthen your resolve. Your {element} essence provides the stability needed to navigate uncertain waters. Remember, every obstacle is a stepping stone to greater wisdom.",
                weight: 2,
            },
        ],
        monthly: [
            {
                ...
                template: "This month marks a significant chapter in {sign}'s cosmic journey. {planet}'s monthly influence brings transformation opportunities that align with your soul's purpose. Your {trait1} and {trait2} qualities will be your guiding stars through this evolutionary period.",
                weight: 3,
            },
            {
                category: 'goals',
                template: "The month ahead is perfectly designed for {sign} to manifest their deepest desires. Your {element} energy builds momentum with each passing day, supported by {planet}'s unwavering guidance. Set intentions that honor both your practical needs and spiritual growth.",
                weight: 3,
            },
        ],
    };
    ...
    static async generate(request: HoroscopeRequest, context: Context): Promise<HoroscopeData> {
        const { sign, type, period, userId, guildId } = request;
        ...
        const templates = this.CONTENT_TEMPLATES[type as keyof typeof this.CONTENT_TEMPLATES] || this.CONTENT_TEMPLATES.daily;
        
        // Select template based on weights and user history
        const selectedTemplate = await this.selectTemplate(templates, userId, context);
        
        // Generate content
        const content = this.fillTemplate(selectedTemplate.template, zodiacInfo, sign);
    ...
    private static fillTemplate(template: string, zodiacInfo: any, sign: string): string {
    ...
    private static getElementActions(element: string): string[] {
        const actions = {
            Fire: ['ignite passion in others', 'lead with confidence', 'inspire bold action', 'create positive change'],
            Earth: ['build lasting foundations', 'provide practical solutions', 'nurture steady growth', 'create tangible results'],
            Air: ['communicate with clarity', 'connect diverse perspectives', 'generate innovative ideas', 'inspire intellectual growth'],
            Water: ['navigate emotional depths', 'provide healing presence', 'trust intuitive wisdom', 'create emotional harmony'],
        };
    ...
    private static selectAdvice(type: string): string {
        const adviceList = this.ADVICE_TEMPLATES[type as keyof typeof this.ADVICE_TEMPLATES] || this.ADVICE_TEMPLATES.daily;
